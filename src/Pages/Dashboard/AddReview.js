    ...
    return (
        <div>
            ...
            <form className='flex flex-col px-5  lg:w-3/6 mx-auto mt-6' onSubmit={handleSubmit(onSubmit)}>
                <input className=' font-bold mb-1 p-3 rounded bg-cyan-100' name='ratings' placeholder='Ratings' {...register("ratings")} required/>
                <textarea className='font-bold mb-1 p-3 rounded bg-cyan-100'
                    name='description' placeholder='Description' {...register("description")} />
                <input className='mb-2 w-50 mx-auto bg-primary mt-2 px-4 py-2 rounded text-large font-semibold cursor-pointer text-white add-product' type="submit" value="Add reviews" />
            </form>
        </div>
    );
};
